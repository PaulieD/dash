...
private:
    ...
    /**
     *
     * @param signOffset What offset to use when validating quorum signature. 0 validates against current quorums, dkginterval against previous quorums
     * @param pend list of pending islocks to validate as well as the node that send those islocks to us
     * @param ban whether or not to bump ban score on invalid islocks. Often due to quorum rotation
     * @return Returns set of bad islocks. These may be islocks that were signed by a previous quorum.
     */
...
public:
    ...
    // Below methods do not immediately return if InstantSend is disabled
